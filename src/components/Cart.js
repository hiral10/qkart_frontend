...
let map = new Map(productsData.map(o => [o["_id"], o]));
return cartData.reduce((acc, o) => {
    let match = map.get(o["productId"]);
  ...
  return (
    <>
      <Box className="cart">
        ...
        <>
        {items.map((product)=>(
      <Box display="flex" alignItems="flex-start" padding="1rem" key={product._id}>
