  ...
  return (
    <Box display="flex" flexDirection="column">
      <TextField
        ...
        onChange={(e) => {
          handleNewAddress({isAddingNewAddress: true,
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
   // console.log("new",newAddress.value)
    try {
     ...
     let response = await axios.post(`${config.endpoint}/user/addresses`,addressdata,{headers})
  ...
  const validateRequest = (items, addresses) => {
    //console.log("res",addresses.selected);
  ...
  //  console.log("balance",localStorage.getItem("balance"),"total-",items)
    else if(localStorage.getItem("balance") < items){
      //console.log("yes")
 ...
 // console.log(addresses)
  const performCheckout = async (token, items, addresses) => {
    //try {
     ...
     // console.log("res",validateRequest(items,addresses));
      // if(addresses.selected===''){
      //   enqueueSnackbar("Please select one shipping address to proceed.",{variant:"error"});
      //  }
      if(validateRequest(items,addresses)===false)
     {
        if(addresses.all.length===0){
     ...
     let response = await axios.post(`${config.endpoint}/cart/checkout`,addressdata,{headers})
    if(validateRequest(items,addresses)){
      ...
      localStorage.setItem("balance",localStorage.getItem("balance")-items);
  ...
  // };
  //setAddresses({...addresses,selected:"el"})
//console.log(addresses);
  return (
    <>
      <Header />
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               ...
               {addresses.all.map((ele)=>(
                     <List key={ele._id} className="address-item">
                    <ListItem key={ele._id} className={addresses.selected === ele._id
                    ? 'selected'
                    : 'not-selected'}
                ...
                <Button name="delete" startIcon={<Delete/>} onClick={()=>deleteAddress(localStorage.getItem("token"),ele._id)}>
                 DELETE </Button>
                 </List>
               ))}
             </Box>
           ...
           {newAddress.isAddingNewAddress===false?
            ...
            <Button
              ...
              variant="contained"
              onClick={()=>performCheckout(token,getTotalCartValue(items),addresses)}
            >
              PLACE ORDER
            </Button>
          </Box>
