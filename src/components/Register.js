...
const Register = () => {
  ...
  const formVal={username:"",password:"",confirmPassword:""};
   ...
   */
    ...
    const apiCall=async(formdata)=>{
      ...
      },setisLoading(true)).then(function (response) {
  ...
  const validateInput = (data) => {
    ...
    enqueueSnackbar('Username is required field', {variant: 'warning'});
     ...
     enqueueSnackbar('Username is < 6 characters', {variant: 'warning'})
      return false;
    }
    else if(data.password.length<6){
      enqueueSnackbar('Password is < 6 characters ', {variant: 'warning'})
       return false;
     }
